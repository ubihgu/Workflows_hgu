...
#    util.log_to_process_file(process_id, ver)
    ...
    error_devices[devicelongid] = list()
    error_devices[devicelongid].append({'id': devicelongid})
    ret = MSA_API.process_content('FAILED', f'Test failed on {error_devices}.', context, True)
    print(ret)
...
ret = MSA_API.process_content('ENDED', 'Apache will be updated on {} running 7.0.106. Errors happened on {}'.format(context['matched_devices'], error_devices), context, True)
print(ret)